...
| Multiplexor | [`herdr/.config/herdr/config.toml`](../herdr/.config/herdr/config.toml) | cromo de herdr sobre Ink-0: texto en tres escalones de grafito (`#cecece` · `#c6c6c6` · `#9e9e9e`), fila activa Ink-3, teal reservado al acento y estados de agente como señal, con glifo propio cada uno (`×` rose blocked · `◐` gold working · `✓` sage done · `○` grafito idle) |
