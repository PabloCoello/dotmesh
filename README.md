...
# Clona el repo (ajusta la ruta según tu sistema: ~/Documentos en Linux con locale es_ES)
...
- Claude Code no permite granularidad fina de comandos bash por agente. Los agentes restrictivos (`security`, `maths`) declaran su scope en las instrucciones del sistema prompt.
...
| Para añadir… | Crea | Y… |
|---|---|---|
| Una skill nueva | `agents/.agents/skills/<nombre>/SKILL.md` | `make restow` |
| Un agente OpenCode | `opencode/.config/opencode/agents/<nombre>.md` | `make restow` |
