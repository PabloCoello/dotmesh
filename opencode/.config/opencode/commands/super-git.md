...
- **Per-slice commits happen wherever the slice is written; sync, push, and PR are a single finalization.** Normally that is the main session, committing each green slice as it lands. On the exception where a phase runs in a `build` subagent, that subagent verifies and commits its own slice. Either way, run the sync, `git push`, and `gh pr create` once at the end, from the main session. PR creation is idempotent: if a PR already exists for the branch, report it instead of opening a duplicate.
- **Keep the live context lean.** The per-commit inspection below (`git diff`, `git diff --staged`, the full `git diff --cached`, the secret scan) costs tokens on every slice, and in a long run those diffs accumulate. Inspect the slice you are committing, not the history you already committed. Delegating each phase does not fix this — measured, an orchestrator peaks at the same context as the inline arm — so when the session really is full, close the slice and hand off.
