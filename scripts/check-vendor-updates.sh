...
is_safe_ref() {
  local ref="$1"
  [[ "$ref" =~ ^(HEAD|refs/(heads|tags)/[A-Za-z0-9._/-]+)$ ]] || return 1
  case "$ref" in
    -*|*..*|*@{*|*//*|*/|*.) return 1 ;;
  esac
  return 0
...
while IFS=$'\t' read -r component local_path upstream_url upstream_ref local_ref notes; do
  ...
  if ! output="$(GIT_TERMINAL_PROMPT=0 GIT_ALLOW_PROTOCOL=https git ls-remote --quiet "$upstream_url" "$upstream_ref" 2>/dev/null)"; then
