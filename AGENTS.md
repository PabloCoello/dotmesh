...
| `herdr/` | `~/.config/herdr/config.toml` | herdr agent multiplexer config (inherits Ghostty's ANSI palette via `[theme] name = "terminal"`). Its agent-state hooks are vendored into the `claude/`, `codex/` and `opencode/` packages, not re-generated by `herdr integration install` |
