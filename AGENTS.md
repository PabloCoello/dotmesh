...
| `ghostty/` | `~/.config/ghostty/config`, `~/.config/ghostty/themes/dotmesh` | Ghostty terminal config + **dotmesh** theme (Ink + syntax; see `docs/DESIGN.md`). Alternative host terminal to Warp |
...
- **Theme colours start from `docs/DESIGN.md`.** The dotmesh visual language (palette, type, syntax map) is the single source of truth for the VS Code theme, Warp theme, Ghostty theme, Starship palette, delta/Git colours and the GNOME desktop retint. Change colours there first, then propagate; keep the surfaces in sync.
