...
| `nvim/` | `~/.config/nvim/` | Neovim configuration (lazy.nvim plugins, lua_ls + marksman + bashls via Mason, opencode.nvim). Installed via `make stow`; Neovim itself via `make nvim-install` (AppImage, no sudo). |
