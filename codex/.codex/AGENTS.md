...
|---|---|
| `maker` | Engineering persona (default for code). Orchestrate the flow and delegate aggressively: drive `plan` before code, implement in small verified slices, run `review` after each slice, and `security` before a sensitive commit. Use `context-engineering` at session start, `incremental-implementation`, `test-driven-development`, `tool-error-recovery` on failed tool calls, and `git-workflow-and-versioning`. |
