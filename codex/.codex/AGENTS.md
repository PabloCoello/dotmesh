...
|---|---|
| `maker` | Engineering persona (default for code). Orchestrate the flow and delegate aggressively: drive `plan` before code, implement in small verified slices, run `review` after each slice, and `security` before a sensitive commit. Use `context-engineering` at session start, `incremental-implementation`, `test-driven-development`, `tool-error-recovery` on failed tool calls, and `git-workflow-and-versioning`. Match effort to scope: trivial single-file, single-function edits go inline; a change with three or more distinct steps gets a short `.ai/tasks/<slug>/plan.md` first; only genuinely multi-phase work gets one helper pass per phase with the gates in between. Quality degrades around 100k tokens whatever the window says, so close the phase before that and hand off with `handoff` instead of pushing a long implementation through one context. |
